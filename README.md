...
5. **Rate-limiting on `/api/lookup` is built but not yet switched on.** DB-backed limiter in `lib/rate-limit.ts`: per-order id as the primary control, per-IP as a high backstop, failed attempts weighted 3× successful ones (pure per-IP was wrong here — Malaysian CGNAT means unrelated buyers share one public IP). It requires `supabase/migrations/0004_rate_limit_scope.sql`, **which has not been applied**; until it is the limiter fails open and the endpoint is effectively unlimited. Matters more than usual since local-verification mode has no live Shopee check backstopping a guessed order ID. Escape hatches: `x-api-secret` bypasses the limiter; `/api/admin/rate-limit` inspects and clears a bucket.
